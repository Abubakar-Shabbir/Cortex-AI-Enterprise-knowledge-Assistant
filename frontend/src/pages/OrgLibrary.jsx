    ...
    <>
            ...
            <div className="mt-4 space-y-1.5">
              {data.add_candidates.length > 0 ? data.add_candidates.map((doc) => (
                <div key={doc.id} className="flex items-center justify-between rounded-lg border border-line px-3 py-2 text-sm dark:border-line-dark">
                  ...
                  <button
                    type="button" onClick={() => toggleOrgLibrary.mutate(doc.id)}
                    className="shrink-0 rounded-lg border border-line px-2.5 py-1 text-xs font-medium text-primary hover:border-primary/30 hover:bg-primary/5 dark:border-line-dark dark:text-primary-soft"
                  >
                    Add to Library
                  </button>
                </div>
      ...
      <SimpleDocumentTable
        ...
        renderExtraActions={data.can_manage ? (doc) => (
          <button type="button" onClick={() => onRemove(doc)} title="Remove from Library" className="rounded-lg p-2 text-muted transition-colors hover:bg-danger/10 hover:text-danger dark:text-muted-dark dark:hover:text-danger-dark">
