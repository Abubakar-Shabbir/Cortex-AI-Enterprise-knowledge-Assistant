...
- **File storage**: stays local filesystem (`MEDIA_ROOT`), unchanged. This assumes the host gives the process a persistent disk (Fly.io's free volume, PythonAnywhere) — a host with a fully ephemeral filesystem (e.g. Render's free web service) will lose uploaded documents on restart/redeploy, so avoid those for this app unless/until object storage is added.
...
- `monitoring_view` — **replaces the old `settings_view`/`/settings/` route.** Same RAG-pipeline-config + DB/pgvector content as before, now also showing background task pool health (see Background Task Execution above), and gated behind `request.user.is_staff` (a real, pre-existing Django field — not a new RBAC system; see Role-based navigation below). Any authenticated user could reach the old settings page; that was never intentional, just never tightened until this rebuild.
